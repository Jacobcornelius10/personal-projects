...
# Function too decrypt a password
...
def view_websites():
    try:
        with open('passwordsl.json', 'r') as data:
            view = json.load(data)
            print("\nWebsites you saved...\n")
            for x in view:
                print(x['website'])
...
# Functions to add "save passwords"
def add_password(website, password):
    # Check if passwordsl.json exists
    if not os.path.exists('passwordsl.json'):
        # If passwords.json doesn't exists, initialize it with an empty list
        data = []
    else:
        # Load existing data from passwords.json
        try:
            with open('passwordsl.json', 'r') as file:
                data = json.load(file)
        except json.JSONDecodeError:
            # handle the case where passwordsl.json is empty or invalid JSON.
            data = []
    # Encrypt the password
    encrypted_password = encrypt_password(cipher, password)

    # create a dictionary to store the website and passoward
    password_entry  = {'website': website, 'password': encrypted_password}
    data.append(password_entry)

    # save the updated list back to passwords.json
    with open('passwordsl.json', 'w') as file:
        json.dump(data, file, indent=4)

# Function to retrieve a saved passwords
